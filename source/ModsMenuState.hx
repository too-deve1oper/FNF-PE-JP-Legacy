...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		DiscordClient.changePresence("MOD選択 | In the Menus", null);
		#end
		Lib.application.window.title = "Friday Night Funkin': Psych Engine-JP v" + MainMenuState.psychEngineJPVersion + " - Mods";
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "MODが見つかりません。\nMODファイルを入れて再起動して下さい。", 48);
		if(FlxG.random.bool(0.1)) noModsTxt.text += '\nBITCH LOLLIHGFWTGYDEUIDYTR'; //meanie
		...
		startX -= 190;
		buttonDisableAll = new FlxButton(startX, 0, "全てOFF", function() {
			for (i in modsList)
			{
				i[1] = false;
			}
		...
		startX -= 190;
		buttonEnableAll = new FlxButton(startX, 0, "全てON", function() {
			for (i in modsList)
			{
				i[1] = true;
			}
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += " (このMODを有効にするにはPE-JPの再起動が必要です!)";
				}
